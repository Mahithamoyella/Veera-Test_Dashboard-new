  ...
  <body>
      ...
      <script>
          ...
          async function fetchUserData() {
              try {
                  const response = await fetch('http://54.209.86.85:3082/api/user', {
                      method: 'GET',
                      credentials: 'include'
                  });
                  ...
                  if (error.message.includes('401') || error.message.includes('403')) {
                      ...
                      window.location.href = 'http://54.209.86.85:8121/login/index.html';
                  } else {
                      const headerUsername = document.getElementById('header-username');
                      const sidebarProfilePicture = document.getElementById('sidebar-profile-picture');
                      const headerProfilePicture = document.getElementById('header-profile-picture');
          ...
          async function logout() {
              try {
                  await fetch('http://54.209.86.85:3082/api/logout', {
                      method: 'POST',
                      credentials: 'include'
                  });
